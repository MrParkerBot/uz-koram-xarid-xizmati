"""The counting behind the Boshqaruv paneli (section 2).

Split out of xarid.reports by TASK-UZK-060. Nothing here changed in the move:
the dashboard grew inside the reports module until it was the larger half of
it, and the two never called each other, so they are two modules that shared
a file rather than one module with two jobs.

Reports answer "how much work stands where" for a table the user filters and
exports. The dashboard answers "how is the department doing" as a fixed set of
indicators on one page. They count different things for different readers, and
only the second is bounded to a single screen.
"""
