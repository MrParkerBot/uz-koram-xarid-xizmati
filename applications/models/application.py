"""The application the department works through, and what it orders.

A record with a life cycle rather than a name: it arrives, it is accepted or
rejected, and an accepted one is given to a specialist to carry out.

OrderLine is here because ApplicationItem is what it was written for. The
purchase application's lines and a contract's rows are built on it too, which
is what keeps one order line the same four columns wherever it is asked for.
"""
